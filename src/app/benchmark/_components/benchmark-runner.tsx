  ...
  return (
    <div className='grid grid-cols-1 gap-6 lg:grid-cols-[1fr_2fr]'>
      <section className='rounded-lg border border-gray-800 bg-gray-900/60 p-5'>
        <h2 className='mb-3 text-sm font-semibold text-gray-300'>测试配置</h2>
        <label className='mb-3 block text-xs text-gray-400'>
          ...
          <textarea
            ...
            rows={6}
            className='w-full rounded border border-gray-700 bg-gray-950 p-2 font-mono text-xs text-gray-100'
          />
        </label>
        <label className='mb-3 block text-xs text-gray-400'>
        ...
        <div className='mb-3 grid grid-cols-2 gap-3 text-xs text-gray-400'>
          <label>
            <span className='mb-1 block'>测试方式</span>
            <select
              value={mode}
              onChange={(e) => setMode(e.target.value as 'models' | 'chat')}
              className='w-full rounded border border-gray-700 bg-gray-950 p-2 text-gray-100'
            >
          ...
          <tbody>
            ...
            {rows.map((r, i) => (
              <tr key={i} className='border-t border-gray-800'>
                <td className='p-3'>
                  <div className='font-mono text-xs text-gray-300'>{r.endpoint}</div>
...
function StatusBadge({ row }: { row: Row }) {
  if (row.status === 'running') return <span className='text-yellow-400'>…</span>
  if (row.status === 'blocked') return <span className='text-red-400'>BLOCKED</span>
  if (row.status === 'error')
    return <span className='text-red-400'>{row.httpStatus ?? 'ERR'}</span>
  if (row.status === 'done')
    return <span className='text-green-400'>{row.httpStatus}</span>
