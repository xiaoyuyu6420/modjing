...
**技术选型服从这个原则。** 如果一个框架很优雅但改一行代码要动半个项目，换掉。
...
**为什么不上复杂架构（微服务/消息队列/K8s）：** MVP 阶段全是负担。SQLite + Next.js 一把梭，一个 `git push` 就上线，改完代码刷新页面就生效。
...
> 中转站行业的关键特性，直接决定了上面的数据模型。开发者需要理解这些**为什么存在**。
